...
def generate_rpn_on_dataset(
    ...
    test_timer = Timer()
    test_timer.tic()
    if _proposal_file_ignored is not None:
        return evaluate_proposal_file(dataset, _proposal_file_ignored, output_dir)
    if multi_gpu:
...
def generate_proposals_on_roidb(
    ...
    for i in range(num_images):
        ...
        _t.tic()
        roidb_boxes[i], roidb_scores[i] = im_proposals(model, im, roidb[i])
        _t.toc()
        if i % 10 == 0:
