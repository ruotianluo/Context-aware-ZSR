...
parser.add_argument('--output_dir', default='./data/vgzsnew/', type=str)
parser.add_argument('--num_objects', default=600, type=int, help="set to 0 to disable filtering")
parser.add_argument('--num_rels', default=100, type=int, help="set to 0 to disable filtering")
parser.add_argument('--output_json', default='instances_vgzsnew', type=str)
...
for img in raw_rel_data:
    for rel in img['relationships']:
        ...
        if len(synsets) == 0 or len(synsets) > 1 or synsets not in top_rel_synsets:
            continue
